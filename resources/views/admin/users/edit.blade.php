...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">Ubah informasi pengguna dan role di bawah ini.</p>
        </div>
    ...
    <div class="bg-white/50 backdrop-blur-3xl lg:bg-white lg:backdrop-blur-none rounded-[1.5rem] border border-blue-100 lg:border-slate-100 shadow-xl shadow-blue-500/10 lg:shadow-sm lg:shadow-black/5 p-6 md:p-8">
        <form action="{{ route('admin.users.update', ['pengguna' => $user->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative" x-data="{ open: false, selected: '{{ old('role', $user->roles->first()?->name) }}', label: '{{ old('role', $user->roles->first()?->name) ? ucfirst(old('role', $user->roles->first()?->name)) : '' }}' }">
