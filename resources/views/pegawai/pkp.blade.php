...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Evaluasi PKP</h2>
            <p class="text-sm text-gray-500 mt-1">Rekapitulasi penilaian Predikat Kinerja Pegawai (PKP) Anda dari tahun ke tahun.</p>
        </div>
    ...
    <div class="bg-white/50 backdrop-blur-3xl lg:bg-white lg:backdrop-blur-none rounded-[1.5rem] border border-blue-100 lg:border-slate-100 shadow-xl shadow-blue-500/10 lg:shadow-sm lg:shadow-black/5 overflow-hidden transition hover:shadow-2xl hover:shadow-blue-500/20 lg:hover:shadow-md lg:hover:shadow-black/10">
        @if($pkpEvaluasi->isEmpty())
            <div class="p-10 text-center text-gray-400 space-y-2">
                <div class="text-3xl mb-2">📊</div>
                <p class="font-bold">Belum Ada Rekap PKP</p>
                ...
                <table class="w-full text-sm text-left">
                    <thead class="bg-blue-200/60 text-xs text-blue-900 uppercase tracking-wider border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-4 lg:px-6 py-3 text-center whitespace-nowrap">Berkas PKP</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($pkpEvaluasi as $pkp)
                            <tr class="hover:bg-white/40 transition">
                                <td class="px-4 lg:px-6 py-3 lg:py-4 font-bold text-gray-800 text-center whitespace-nowrap">
                                    {{ $pkp->tahun_penilaian }}
                                    @if($pkp->tahun_penilaian == $tahunBerjalan)
                                ...
                                <td class="px-4 lg:px-6 py-3 lg:py-4 text-center whitespace-nowrap">
                                    @php
                                        $badgeColor = $colors[$pkp->predikat] ?? 'bg-gray-100 text-gray-800';
                                    @endphp
                                ...
                                <td class="px-4 lg:px-6 py-3 lg:py-4 text-center whitespace-nowrap">
                                    @if(in_array($pkp->predikat, ['Baik', 'Sangat Baik']))
