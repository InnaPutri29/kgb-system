            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="relative p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50/50 rounded-xl transition duration-200 focus:outline-none">
                        ...
                        <div x-show="open" @click.outside="open = false" x-cloak
                             ...
                             class="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 origin-top-right">
                            <div class="px-4 py-2.5 border-b border-slate-50 flex items-center justify-between bg-slate-50/50 rounded-t-2xl">
                                <span class="text-xs font-bold text-slate-800">Notifikasi ({{ $unreadCount }})</span>
                                @if($unreadCount > 0)
                                    <form method="POST" action="{{ route('notifications.read-all') }}">
                                        @csrf
                                        <button type="submit" class="text-[10px] text-indigo-600 hover:text-indigo-800 font-bold hover:underline">Tandai semua dibaca</button>
                                    </form>
                                @endif
                            </div>
                            <div class="max-h-64 overflow-y-auto divide-y divide-slate-50">
                                ...
                                @else
                                    @foreach(auth()->user()->notifications->take(10) as $notification)
                                        <div class="p-3.5 hover:bg-slate-50/50 transition flex items-start justify-between gap-2.5 {{ $notification->unread() ? 'bg-indigo-50/10' : '' }}">
                                            ...
                                            @if($notification->unread())
                                                <form method="POST" action="{{ route('notifications.read', $notification->id) }}" class="shrink-0">
                                                    @csrf
                                                    <button type="submit" class="p-1 hover:bg-slate-100 text-indigo-500 hover:text-indigo-700 rounded-md transition focus:outline-none" title="Tandai dibaca">
